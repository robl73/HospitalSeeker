...
(function(global) {
	...
	var messageResource = (function() {
		...
		function defaultFileNameResolver(module, locale) {
			...
			return (locale && typeof locale === 'string') ? module + '_'
					+ locale : module;
		}
		...
		/**
		 * Get a valid locale name. If null is passed, returns default or
		 * current configured locale.
		 * 
		 * @param {String}
		...
		function convertUnicodeString(str) {
			var convertedText = str.replace(/\\u[\dA-Fa-f]{4}/g, function(
					unicodeChar) {
				return String.fromCharCode(parseInt(unicodeChar.replace(/\\u/g,
						''), 16));
			});
		...
		return {
			 ...
			 * @param {String}
			 *            [config.filePath] - Path(directory) of message
			 *            resource files.
			 * @param {String}
			 *            [config.fileExtension = .properties] - File extension
			 *            of message resource files.
			 * @param {String}
			 *            [config.defaultLocale = en_US] - Default locale.
			 * @param {Function}
			 *            [config.fileNameResolver = defaultFileNameResolver] -
			 *            Specify custom file name resolver.
			 * @param {Function}
			 *            [config.ajaxFunction = ajaxGet] - Specify custom ajax
			 *            function for loading files. The function should accept
			 *            only 2 arguments : 1. Url/path of the file. 2.
			 *            Callback with response text as argument.
			 * @param {Boolean}
			...
			init : function(config) {
				...
				fileNameResolver = config.fileNameResolver
			...
			/**
			 * Set current locale to be used. This configured locale will be
			 * used by load and get functions if locale not specified. If
			 * current locale is not set config.defaultLocale will be used as
			...
			/**
			 * Load a message resource file. The file name is constructed based
			 * on the given module name and locale.
			 * 
			 * File name is constructed with default configuration in different
			 * cases as follows :
			 * 
			 * case 1 : Module name and locale can be empty or null, then the
			 * file name will be resolved to _default.properties. case 2 :
			 * Module name - HomePage and locale - empty, then file name -
			 * HomePage.properties. case 3 : Module name - empty and locale -
			 * en_US, then file name - _default_en_US.properties. case 4 :
			 * Module name - HomePage and locale - en_US, then file name -
			 * HomePage_en_US.properties.
			 * 
			 * The above file name construction logic can be overriden by
			 * configuring custom fileNameResolver functions.
			 * 
			 * @param {String |
			 *            Array} [module = DEFAULT_MODULE_NAME] - Module name or
			 *            list of module names.
			 * @param {String}
			 *            [locale] - Locale identifier like en_US. Configured
			...
			load : function(module, callback, locale) {
				...
				// no need to pass a valid locale as locale, locale can be
				// avoided
				// in file name for default locale
				fileLocale = (validLocale === defaultLocale) ? locale
						: validLocale;
			 ...
			 *            current locale will be used if not given.
			 * @param {String}
			 *            defaultValue - Default value to return if value not
			 *            found. If defaultValue is empty key will be used as
			 *            defaultValue.
			...
			get : function(key, module, locale, defaultValue) {
				var validModule, validLocale, moduleObj, value = defaultValue
						|| key;
