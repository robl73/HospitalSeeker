...
public class MapController {
	...
	public String newHospitalFromMap(@RequestParam("hospjs") String data, RedirectAttributes redirectAttributes) {
		try {
	...
	public String saveHospital(@Valid @ModelAttribute("hospitalDTO") HospitalDTO hospitalDTO, BindingResult result,
			RedirectAttributes redirectAttributes, Locale locale) {
		if (result.hasErrors()) {
			return "mapadmin";
		}
		service.save(hospitalDTO);
		redirectAttributes.addFlashAttribute("message",
				messageSource.getMessage("admin.hospital.new.added", null, locale) + hospitalDTO.getName());
	...
	public String editHosital(@PathVariable long id, ModelMap model) {
		...
		HospitalDTO hospitalDTO = service.toHospitalDTO(hospital);
	...
	public String updateHospital(@ModelAttribute("hospitalDTO") @Valid HospitalDTO hospitalDTO, BindingResult result,
			@PathVariable long id, RedirectAttributes redirectAttributes, Locale locale) {
		if (result.hasErrors()) {
			return "mapadmin";
		}
		service.update(hospitalDTO);
		redirectAttributes.addFlashAttribute("message",
				messageSource.getMessage("admin.hospital.new.updated", null, locale) + hospitalDTO.getName());
		return "redirect:/admin/map/listhospitals";
	}

	@PreAuthorize("hasRole('ADMIN')")
	@RequestMapping(value = { "/admin/map/deletehospital/{id}" }, method = RequestMethod.POST)
	public String deleteHospital(@PathVariable long id, RedirectAttributes redirectAttributes, Locale locale) {
		Hospital hospital = service.getById(id);
		service.delete(hospital);
		redirectAttributes.addFlashAttribute("message",
				messageSource.getMessage("admin.hospital.new.deleted", null, locale) + hospital.getName());
		return "redirect:/admin/map/listhospitals";
	}
